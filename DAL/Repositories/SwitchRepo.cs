...
namespace DAL.Repositories
{
    class SwitchRepo : IRepository<Switch>
    {
        ...
        public Switch Read(int id)
        {
            using (var ctx = new CADBContext())
            {
                var a = ctx.Switches.Include(y => y.Ports).Include(c=> c.Customer).FirstOrDefault(x => x.Id == id);
                return a;
            }
        }
        ...
        public List<Switch> ReadAll()
        {
            using (var ctx = new CADBContext())
            {
                var a = ctx.Switches.Include(y => y.Ports).Include(c => c.Customer).ToList();
                return a;
            }
        }
        ...
        public List<Switch> ReadAllWithFk(int id)
        {
            using (var ctx = new CADBContext())
            {
                var afk = ctx.Switches.Include(y => y.Customer).Include(y => y.Ports).Where(x => x.Customer.Id == id).ToList();
                return afk;
            }
        }
